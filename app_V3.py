...
def dashboard():
    ...
    return render_template_string(
        ...
        df=records,          # records list (table uses it directly)
        summary=summary,     # for charts
        total_records=total_records
    )

@app.route("/admin/farmer/<farmer_id>")
def admin_view_farmer(farmer_id):
    user = current_user()
    if not user or user['role'] != 'admin':
        return redirect(url_for("login"))

    records = load_records(farmer_id)
    total_records = len(records)
...
def admin():
    ...
    response = supabase.table("insect_records").select("*").execute()
    records = response.data or []
    ...
    if len(records) > 0:
        ...
        # Farmer Activity Summary (used in the new table)
        farmer_summary = (
            df.groupby("farmer_id")["count"]
              .sum()
              .reset_index()
              .to_dict(orient="records")
        )

        # Insect Summary (for pie chart)
        insect_summary = (
            df.groupby("insect")["count"]
              .sum()
              .reset_index()
              .to_dict(orient="records")
        )
    ...
    return render_template_string(
        ADMIN_HTML,
        farmers=farmer_summary,   # 👈 used in Farmer Table
        summary=insect_summary,   # 👈 used in Insect Pie Chart
        devices=devices
    )
...
def admin_view_farmer(farmer_id):
    ...
    return render_template_string("""
    <h2>Data for {{ farmer_id }}</h2>
    <pre>{{ farmer_df.to_dict(orient='records') }}</pre>
    <a href="/admin">Back</a>
    """, farmer_id=farmer_id, farmer_df=farmer_df)
...
<body>
    ...
    <div class="container">
        ...
        <div class="table-container">
            <h2 class="section-title"><i class="fas fa-users"></i> Farmer Activity Summary</h2>
            <table>
                <thead>
                    <tr>
                        <th>Farmer ID</th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ row.count }}</td>
                        <td>
                            <a href="/admin/farmer/{{ row.farmer_id }}" class="btn-secondary">
                                View Details
                            </a>
                        </td>
                    </tr>
